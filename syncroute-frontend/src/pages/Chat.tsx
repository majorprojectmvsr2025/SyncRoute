...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    const onMessage = (message: Message) => {
      ...
      if (msgRideId === prevRideId.current) {
        setMessages(prev => {
          // Deduplicate: skip if we already have this message ID or a temp version
          if (prev.some(m => m._id === message._id)) return prev;
          // Also remove any temp message with same text (optimistic update replacement)
          const withoutTemp = prev.filter(m => !(m._id.startsWith("temp-") && m.text === message.text));
  ...
  const sendMessage = async () => {
    ...
    setInput("");
    try {
      // Use REST API to send — the socket event will deliver it to the other user
      // We add it locally immediately for instant feedback (optimistic update)
      const tempMsg: Message = {
        _id: `temp-${Date.now()}`,
        ride: activeConvo.rideId,
        sender: { _id: user!._id, name: user!.name, photo: user?.photo },
        receiver: activeConvo.otherUser,
        type: "text",
        text,
        read: false,
      ...
      setMessages(prev => prev.map(m => m._id === tempMsg._id ? { ...sent, sender: tempMsg.sender, receiver: tempMsg.receiver } : m));
      
      setConversations(prev => prev.map(c =>
        c.rideId === activeConvo.rideId
            ...
            <div className="border-t border-border p-3 shrink-0">
              <div className="flex items-center gap-2">
                <ShareLocationButton
                  ...
                  onShared={(msg) => {
                    setMessages(prev => {
                      // Don't add if already present (socket may have already added it)
                      if (prev.some(m => m._id === (msg as any)._id)) return prev;
