...
import {
  ...
  CheckCircle2, Route, ChevronRight, Calendar, Bell,
} from "lucide-react";
import { Link, useNavigate } from "react-router-dom";
import { statsAPI, ridesAPI, bookingsAPI } from "@/lib/api";
...
const Index = () => {
  ...
  useEffect(() => {
    ...
    // Helper function to check if ride is still relevant (not past 3 hours after departure)
    const isRideStillRelevant = (ride: any) => {
      ...
      const hoursSinceDeparture = (now.getTime() - rideDateTime.getTime()) / (1000 * 60 * 60);
      
      // Hide ride if more than 3 hours have passed since departure time
      return hoursSinceDeparture < 3;
    };

    // Fetch today's rides for logged-in user
    const fetchTodayRides = async () => {
      ...
      try {
        ...
        const todayDriverRides = myRides.filter((r: any) => 
          r.date === today && 
          r.status === "active" && 
          isRideStillRelevant(r)
        );
        ...
        const allTodayRides = [...todayDriverRides, ...todayBookings].sort((a, b) => 
          (a.departureTime || "00:00").localeCompare(b.departureTime || "00:00")
        );
        
        setTodayRides(allTodayRides);
      } catch (error) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1">
        ...
        {/* ━━ Today's Rides Banner (for logged-in users) - Shown below search ━━━━━━━━━━━━━ */}
        {user && todayRides.length > 0 && (
          <section className="bg-primary/5 border-y border-primary/10">
            <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4">
              <div className="flex items-center gap-2 mb-3">
                <Calendar className="h-4 w-4 text-primary" />
                <span className="text-sm font-semibold text-primary">Today's Rides</span>
              ...
              <div className="flex gap-3 overflow-x-auto pb-1 -mx-1 px-1">
                {todayRides.map((ride: any) => (
                  <button
                    ...
                    className="flex-shrink-0 bg-card border border-border rounded-lg p-3 hover:border-primary/50 transition-colors text-left min-w-[260px]"
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-mono font-semibold">{ride.departureTime || "N/A"}</span>
                      {ride._isBooking ? (
                        <span className="text-[10px] bg-blue-500/10 text-blue-500 px-2 py-0.5 rounded font-medium">
                          Passenger
                        </span>
                      ) : (
                        <span className="text-[10px] bg-green-500/10 text-green-500 px-2 py-0.5 rounded font-medium">
                          Driver
                        </span>
                      )}
                    </div>
                    <div className="text-sm font-medium truncate">
                      {ride.from?.name || ride.source?.name || "Start"} → {ride.to?.name || ride.destination?.name || "End"}
                    </div>
                    <div className="text-xs text-muted-foreground mt-1">
                      {ride.vehicleType} · ₹{ride.price}/seat
                    </div>
                  </button>
                ))}
              </div>
    ...
    >
      ...
      <div className="flex items-center justify-between pt-3 border-t border-border">
        <div className="flex items-center gap-2">
          {driver.photo ? (
            <img src={driver.photo} className="h-6 w-6 rounded-full object-cover" alt="" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
          ) : (
          ...
          {/* Only show rating if driver has actual reviews */}
          {(driver.reviewStats?.totalReviews > 0 || driver.reliabilityScore?.totalRatings > 0) && (
            <span className="flex items-center gap-0.5 text-xs text-muted-foreground">
              <Star className="h-3 w-3 fill-amber-400 text-amber-400" /> {driver.reviewStats?.avgStars?.toFixed(1) || driver.reliabilityScore?.avgRating?.toFixed(1)}
            </span>
          )}
        </div>
