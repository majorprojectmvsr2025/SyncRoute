...
router.post("/search", optionalAuth, async (req, res) => {
  try {
    ...
    for (const ride of rides) {
      ...
      // Use route path matching if route has at least 2 coordinate points
      if (routeCoords && routeCoords.length >= 2) {
        const pickup = closestRoutePoint(routeCoords, pLat, pLng);
        const drop   = closestRoutePoint(routeCoords, dLat, dLng);

        console.log(`    Route has ${routeCoords.length} points. Pickup dist: ${Math.round(pickup.dist)}m idx:${pickup.idx}, Drop dist: ${Math.round(drop.dist)}m idx:${drop.idx}`);

        // Check if pickup and drop are close enough to the route
        if (pickup.dist > NEAR_ROUTE_M || drop.dist > NEAR_ROUTE_M) {
          console.log(`    [SKIP] Too far from route (max ${NEAR_ROUTE_M}m)`);
          continue;
        }
        
        // Allow pickup.idx == drop.idx for very close points, only reject if pickup comes AFTER drop
        // This ensures the travel direction is correct (from start toward end of route)
        if (pickup.idx > drop.idx) {
          console.log(`    [SKIP] Wrong direction (pickup idx ${pickup.idx} > drop idx ${drop.idx})`);
          continue;
        }
        ...
        console.log(`    No route path, using fallback matching`);
        ...
        console.log(`    Fallback distances: pickup ${Math.round(pickupDistM)}m, drop ${Math.round(dropDistM)}m (max ${FALLBACK_M}m)`);
        if (pickupDistM > FALLBACK_M || dropDistM > FALLBACK_M) {
