...
const { trackReviewSubmitted } = require("../utils/behaviorTracker");

const router = express.Router();

// Helper function to update user's review stats and rating
  ...
  try {
    ...
    if (totalReviews === 0) {
      await User.findByIdAndUpdate(userId, {
        $set: {
          ...
          "reviewStats.avgStars": 0,
          "reliabilityScore.totalRatings": 0,
          "reliabilityScore.avgRating": 0
        }
      });
      return;
    }
    ...
    await User.findByIdAndUpdate(userId, {
      $set: {
        ...
        "reviewStats.avgStars": roundedAvg,
        "reliabilityScore.totalRatings": totalReviews,
        "reliabilityScore.avgRating": roundedAvg
      }
    });
    
    console.log(`[Reviews] Updated stats for user ${userId}: ${totalReviews} reviews, ${roundedAvg} avg`);
  } catch (error) {
    console.error(`[Reviews] Failed to update stats for user ${userId}:`, error);
  }
}
...
router.post("/", protect, async (req, res) => {
  try {
    ...
    // Update reviewee's rating stats (important for consistent ratings across the app)
    await updateUserReviewStats(reviewee);
    ...
    // Track review behavior for ML (async)
    trackReviewSubmitted(req.user._id, booking, booking.ride, ratingNum);
