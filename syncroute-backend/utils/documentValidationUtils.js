...
// ────────────────────────────────────────────────────────
const DL_PATTERNS = [
  // Standard format: SS-RR-YYYYNNNNNNN
  /^([A-Z]{2})[- ]?(\d{2})[- ]?(\d{4})[- ]?(\d{7})$/i,
  // Alternate: SS-RR-YYYY-NNNNNN (6-8 digits)
...
function validateDLFormat(dlNumber) {
    ...
    if (match) {
      ...
      if (year < 1980 || year > currentYear) {
        return {
          ...
          year,
          serial,
          message: `Invalid issue year in DL number: ${year}`,
        };
      }
  ...
  return {
    valid: false,
    dlNumber: cleaned,
    message: "Driving license number does not match valid Indian DL format (SS-RR-YYYYNNNNNNN)",
  };
}
